...
public class PledgeContractController {
    ...
    public ResponseEntity<?> updatePledgeContract(
        ...
        try {
            PledgeContract updatedContract = pledgeContractService.updatePledge(
                    id,
                    payloadJson,
                    portraitFile,
                    attachmentFiles
            ...
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("error", "Lỗi xử lý cập nhật hợp đồng: " + e.getMessage()));
        }
    }
    ...
    @GetMapping()
    public  ResponseEntity<?> searchPledgesGet(@ModelAttribute PledgeSearchRequest request){
        Page<PledgeContractListResponse> pledgeContractListResponses = pledgeContractService.searchPledges(request);
        return restEntityRes.setHttpStatus(HttpStatus.OK).setDataResponse(pledgeContractListResponses).getResponse();
    }
}